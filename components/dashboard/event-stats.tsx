...
import { Calendar, Users, Gift, DollarSign } from "lucide-react"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"
import { mockGuests, mockGifts } from "@/lib/mock-data"
...
export function EventStats({ eventId }: EventStatsProps) {
  // Em uma aplicação real, buscaríamos esses dados do banco de dados
  ...
  const totalGiftItems = gifts.reduce((acc, gift) => acc + gift.quantity, 0)
  const reservedGifts = gifts.reduce((acc, gift) => acc + (gift.reserved || 0), 0)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Presentes Escolhidos</CardTitle>
